...
import { SPGNFTContractAddress, client, createCommercialRemixTerms, defaultLicensingConfig, RoyaltyPolicyLAP } from "./utils";
...
export async function mintAndRegisterOutline(inputData : string) {
  ...
  const commercialRemixTerms = createCommercialRemixTerms({commercialRevShare: 50, defaultMintingFee: 0})
  const response = await client.ipAsset.mintAndRegisterIpAssetWithPilTerms({
    spgNftContract: SPGNFTContractAddress,
    licenseTermsData: [{terms: commercialRemixTerms, licensingConfig: defaultLicensingConfig}], // IP already has non-commercial social remixing terms. You can add more here.
